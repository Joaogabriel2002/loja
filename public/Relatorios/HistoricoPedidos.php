    ...
    <div id="detalhesModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-lg p-8 shadow-xl max-w-2xl w-full">
            <div class="flex justify-between items-center border-b pb-3 mb-4">
                <h3 id="modalTitle" class="text-2xl font-bold">Detalhes do Pedido</h3>
            ...
            <div id="modalBody" class="max-h-96 overflow-y-auto">
                <!-- Conteúdo dos itens do pedido será inserido aqui -->
            </div>
        </div>
    </div>
        ...
        async function verDetalhes(pedidoId) {
            ...
            try {
                ...
                itens.forEach(item => {
                    ...
                    total += subtotal;
                    content += `<tr class="border-b"><td class="p-2">${item.nome}</td><td class="p-2 text-center">${item.quantidade}</td><td class="p-2 text-right">R$ ${parseFloat(item.preco_unitario_momento).toFixed(2)}</td><td class="p-2 text-right font-semibold">R$ ${subtotal.toFixed(2)}</td></tr>`;
                });
